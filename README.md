# Olá, sou o Mário Belim 👋

Estudante de **Engenharia Informática** (2º ano) na Universidade da Madeira.
Interessado em sistemas, redes e — a longo prazo — **cibersegurança**.

---

## Sobre mim

- 2º ano de Engenharia Informática na Universidade da Madeira
- Gosto especialmente das cadeiras de sistemas e redes
- Quero seguir mestrado em Cibersegurança
- Este perfil é onde vou guardando os projectos que faço ao longo do curso

---

## Cadeiras concluídas

| Área | Cadeiras |
|---|---|
| Programação | Paradigmas da Programação, POO |
| Sistemas | Sistemas Digitais, Arquitectura de Computadores |
| Dados | Algoritmos e Estruturas de Dados, Bases de Dados |
| Redes | Redes e Comunicação de Dados |
| Matemática | Cálculo I, Probabilidade e Estatística, Métodos Numéricos, Matemática Discreta |
| Teoria | Lógica Computacional, Teoria e Fundamentos da Computação |

---

## Skills

### Linguagens que uso
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Verilog](https://img.shields.io/badge/Verilog-FF6B35?style=flat&logo=intel&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black)

### Ferramentas
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![NetBeans](https://img.shields.io/badge/NetBeans-1B6AC6?style=flat&logo=apachenetbeanside&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat&logo=visualstudiocode&logoColor=white)

---

## Projectos académicos

### ⚙️ maquina-lavar-verilog
Controlador de máquina de lavar implementado em Verilog como máquina de estados.
Inclui ciclo completo (água, aquecimento, detergente, lavagem, enxaguamento, centrifugação)
e funcionalidades extra: protecção contra falha de energia e atraso de início programável.

`Verilog` `Sistemas Digitais` `Máquinas de Estado` `ISE Xilinx`

### 🕵️ sherlock-holmes-text-adventure
Jogo de aventura textual em Java ambientado no Funchal. O jogador assume o papel de
Sherlock Holmes para resolver um mistério no porto industrial da cidade.
Desenvolvido com princípios SOLID, Design Pattern Command e POO avançada.

`Java` `POO` `Design Patterns` `SOLID` `NetBeans`

### 🌐 dev-web
Projecto da cadeira de Desenvolvimento Baseado na Web (em curso).

`HTML` `CSS` `JavaScript`

---

## Estatísticas GitHub

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=mariobelim3&show_icons=true&theme=dark&hide_border=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=mariobelim3&layout=compact&theme=dark&hide_border=true)

---

## Contacto

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/mariobelim)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto=mario@email.com)
